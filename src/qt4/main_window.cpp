...
MainWindow::init()
{
    ...
    //Return true if the auto_image_save is active
    if ( Options::instance().autoImageSave() )
    {
        //calls a slot (saveImageAndQuit) after a given time interval (500)
...
{
    //in "file" menu : open rcg file, then open a window where we choose the rcg file
    ...
    //then call the openRCG function
    connect( M_open_rcg_act, SIGNAL( triggered() ), this, SLOT( openRCG() ) );
    this->addAction( M_open_rcg_act );
    //
    //in "file" menu :Save rcg file as, then open a window where we choose to save rcg file (when there is monitor view data)
...
*/
/*
* Create the canvas of the field
* FieldCanvas class :
...
MainWindow::createFieldCanvas()
{
    ...
    {
        // display the menu "open rcg file" - "connect" when right clic
...
// #endif
    ...
    {
        // menu displayed when during a match, left clic, change Playmode (in coach mode)
    ...
    {
        // menu displayed when during a match, left clic
        QMenu * menu = new QMenu( M_field_canvas );
        menu->addAction( M_kick_off_act );
        menu->addSeparator();
...
/*
* view_config_dialog class :
*   zoom controls, object size controls,focus controls, players controls, field style controls ...
* all that concern the appearence of the canvas
*/
void
...
/*
* if we want to open an already done rcg
*/
void
MainWindow::openRCG()
{
...
*/
//Called when we do "File - Save image"
void
MainWindow::showImageSaveDialog()
{
    //Stop the log player (not in live mode anymore + timer stop)
    M_log_player->stop();
